...
- **但这里有个反直觉发现，我认为比「机制起效」更重要**：compact_off 反而**更快（13 < 16 轮）、产出质量更高（33 个测试 vs 16 个）**。
  为什么？压缩把历史揉成摘要，丢掉了细节；模型在压缩后「忘了」一些已写的测试用例，导致补写/返工，最终测试反而更少、轮数更多。
  **这说明压缩不是免费的午餐——它在「不压会撞墙」时是救命的，但只要窗口还够用，它就是净负担（信息损失 + 时间 overhead + 产出缩水）。**

这个发现直接定义了我机制①的设计取向：**能不压就不压、要压就晚压（75%）且压得保守（只压旧历史、保留最近 3 轮、重要状态放文件系统）**——
本质是把压缩的代价压到最小，只在真要撞墙时才动用它。（这个结论在 DeepSeek 上更夸张，见「跨模型对比」。）
...
同一套评测在 K2.7 和 DeepSeek（v4-pro）上各跑一遍，结论不是「两家都很好」，而是一个**反直觉但重要**的分层。
两个模型上都复现了「**窗口够用时，压缩是净负担**」：

| | K2.7 compact_on | K2.7 compact_off | DeepSeek compact_on | DeepSeek compact_off |
|---|---|---|---|---|
| S1 完成 | ✅ 16 轮 / 16 测试 | ✅ 13 轮 / 33 测试 | ❌ 24 轮撑满未完成 | ✅ 15 轮 |
| 压缩次数 | 1 | 0 | 3 | 0 |

- K2.7：开压缩反而更慢（16>13 轮）、产出更少（16<33 测试）——压缩损失细节导致返工，但还能完成。
- DeepSeek：更脆弱，3 次压缩直接导致**任务做不完**（24 轮撑满），关掉压缩 15 轮一气呵成。

两个工程结论：

1. **机制 ②③④ 是模型无关的硬保证**：状态落盘、重试、中断在两个模型上行为完全一致（S2 都是 on 完成/off 崩溃，S5 都正确采纳 steering）。它们是 harness 层的确定性保证，不依赖模型聪不聪明。
2. **机制 ① 的效果与模型强相关，且有副作用**：压缩质量 = 模型的摘要能力，弱模型上频繁压缩是净负担甚至致命。
   这反过来印证我机制①的设计选择——**75% 才触发、只压旧历史保留最近 3 轮、重要状态放文件系统**——本质都是在压缩的收益（不撞墙）和代价（信息损失 + 时间 + 产出缩水）之间尽量延后、减小那个代价。
